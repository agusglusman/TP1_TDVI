...
mejor_arbol <- function(a,b,c,x,y,z, train_data, valid_data){
  ...
  for (i in x:a) {
    ...
    for (j in y:b) {
      ...
      for (k in z:c) {
        tree_mobible1 <- rpart(
          formula = loan_status ~ person_age + person_gender + person_education + person_income + person_emp_exp + 
                                  person_home_ownership + loan_amnt + loan_intent + loan_int_rate + loan_percent_income +
                                  cb_person_cred_hist_length + credit_score + previous_loan_defaults_on_file, 
          data = train_data, 
          method = "class", minsplit = k, minbucket = j, maxdepth = i, cp = 0, xval = 0
        )
        
        # Calcular AUC validación
        probabilities <- predict(tree_mobible1, newdata = valid_data, type = "prob")
        AUC_value_valid <- MLmetrics::AUC(
          y_pred = probabilities[, "Rechazado"], 
          y_true = as.numeric(valid_data$loan_status == "Rechazado")
        )
        
        # Guardar mejor AUC
        if (AUC_value_valid > mejor_arbol_valor) {
          mejor_arbol_valor <- AUC_value_valid
          mejor_arbol_MD <- i
          mejor_arbol_MB <- j
          mejor_arbol_MS <- k
        }
  ...
  # Crear dataframe con resultados (solo validación)
  result <- data.frame(
    aucroc   = mejor_arbol_valor,
    maxdepth = mejor_arbol_MD,
    minbucket= mejor_arbol_MB,
    minsplit = mejor_arbol_MS
  )
...
prueba1 <- mejor_arbol(a=5, b=30, c=20,    x=10, y=20,   z=1,    train_data=train_data, valid_data=valid_data)
prueba2 <- mejor_arbol(a=15, b=30, c=100, x=10, y=20, z=10, train_data=train_data, valid_data=valid_data)
prueba3 <- mejor_arbol(a=10, b=1020, c=1020,x=1,  y=1000, z=1000, train_data=train_data, valid_data=valid_data)
prueba4 <- mejor_arbol(a=12, b=30, c=50, x=10, y=20, z=5,   train_data=train_data, valid_data=valid_data)
prueba5 <- mejor_arbol(a=1,  b=1, c=1, x=1, y=1,  z=1,  train_data=train_data, valid_data=valid_data)
...
kable(tabla_resultados, align = "c", caption = "Comparación de todas las pruebas (Validación)") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed"))
```
```{r}

random_search <- function(train_data, valid_data, n = 100, seed = 777) {
  ...
  set.seed(seed) # replicabilidad
  
  for (i in 1:n) {
    # hiperparámetros aleatorios
    maxdepth  <- sample(10:15, 1)
    minsplit  <- sample(5:50, 1)
...
rs <- random_search(train_data, valid_data, n = 150)  
